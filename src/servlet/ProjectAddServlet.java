...
public class ProjectAddServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//int memeber1_id = Integer.parseInt(request.getParameter("memeber1_id"));
		//int memeber2_id = Integer.parseInt(request.getParameter("memeber2_id"));
		//int memeber3_id = Integer.parseInt(request.getParameter("memeber3_id"));
		...
		boolean flag=dao.projectAdd(project);
		PrintWriter out = response.getWriter();
		//System.out.println(flag);
		if(flag) {			
			out.print("<script>alert('添加项目信息成功!');window.location.href='admin/ProjectManage/addProject.jsp'</script>");
			
		}else {
			out.print("<script>alert('添加项目信息失败!');window.location.href='admin/ProjectManage/addProject.jsp'</script>");
		}
	}
