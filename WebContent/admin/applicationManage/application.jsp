  ...
  <section id="container" class="">
      ...
      <!--header end-->
          <section class="wrapper">            
			  ...
			  <div class="row">
				<div class="col-lg-12">
					<h3 class="page-header"><i class="fa fa-th-list"></i> 显示未审核的申请信息</h3>
					<ol class="breadcrumb">
						<li><i class="fa fa-home"></i><a href="<%=request.getContextPath() %>/admin/Home.jsp">Home</a></li>
						<li><i class="icon_genius"></i>审核信息管理</li>
						<li><i class="fa fa-th-list"></i>显示未审核的申请信息</li>				  	
					</ol>
				</div>
			</div>
        ...
        <%List<Application> application2=(List<Application>)request.getAttribute("application2");    
      if(application2!=null&&application2.size()>0){
    	  int i;
      for(i=0;i<application2.size();i++){
      %>                      
                              <tr>
                                 <td><%=application2.get(i).getName() %></td>
                                 <td><%=application2.get(i).getSex() %></td>
                                 <td><%=application2.get(i).getNativeplace() %></td>
                                 <td><%=application2.get(i).getBirthday() %></td>
                                 <td><%=application2.get(i).getEducation() %></td>                                 
                                 <td><%=application2.get(i).getTitle() %></td>
                                 <td><%=application2.get(i).getQuatime() %></td>
                                 <td><%=application2.get(i).getGraschool() %></td>
                                 <td><%=application2.get(i).getGratime() %></td>
                                 <td><%=application2.get(i).getMajor()%></td>    
                                 <td><%=application2.get(i).getWorkingtime()%></td>
                                 <td><%=application2.get(i).getCurrentmajor()%></td>                   
                                 <td>
                                    <div class="btn-group">
                                      <a class="btn btn-success" href="AuditApplicationServlet?id=<%=application2.get(i).getId()%>">
                                      	 <i class="icon_check_alt2"></i>
									  </a>
                                      <a class="btn btn-danger" href="AuditNoApplicationServlet?id=<%=application2.get(i).getId()%>"><i class="icon_close_alt2"></i></a>
                                  </div>
                                  </td> 
                               </tr> 
		<%}} %>
